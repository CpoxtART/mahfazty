...
const CHANGELOG = [
  ...
  },
  {
    ...
    items: [
      { ar: 'إصلاح عالي: فئة "تحويل" 🔁 كانت تظهر وتُختار بشكل عادي في شبكة الفئات عند إضافة أو تعديل أي معاملة — رغم أنها مخصَّصة حصرًا للأرجل الناتجة تلقائيًا عن نافذة "تحويل بين محافظ". أي معاملة عادية (مصروف أو دخل) يُختار لها "تحويل" بالخطأ كانت تُحفظ بشكل صحيح ماليًا، لكن تختفي بصمت من كل الرسوم البيانية والتحليلات والميزانيات إلى الأبد — بلا أي تحذير. أُزيلت الفئة من قائمة الاختيار في كلا النموذجين (الإضافة والتعديل).', en: 'High fix: the "Transfer" 🔁 category appeared as a normal, selectable option in the category grid when adding or editing any transaction — despite being reserved exclusively for the legs automatically generated by the dedicated "Transfer between wallets" flow. An ordinary expense or income mistakenly tagged "Transfer" still saved correctly financially, but silently vanished from every chart, analytics figure, and budget calculation forever — with zero warning. Removed the category from the selectable list in both the add and edit forms.' },
      { ar: 'إصلاح متوسط: نافذة التحويل بين المحافظ كانت تسمح باختيار **نفس المحفظة** كمصدر ووجهة معًا مباشرة من القائمتين المنسدلتين — يُكتشف الخطأ فقط عند الضغط على "تحويل" برسالة خطأ متأخرة، بدون أي إشارة بالقائمة نفسها توضح أي جانب يحتاج تغييرًا. الآن تستبعد كل قائمة اختيار المحفظة المختارة بالجانب الآخر تلقائيًا.', en: 'Medium fix: the transfer-between-wallets modal allowed picking the SAME wallet as both source and destination directly from the two dropdown pickers — the conflict was only caught after tapping "Transfer," with a late error message and no indication in the pickers themselves of which side needed changing. Each picker now automatically excludes whatever the other side currently has selected.' },
      { ar: 'إصلاح متوسط: نسخ التقرير الشهري للحافظة كان بإمكانه الفشل بصمت تام (بلا أي رسالة أو تنزيل احتياطي) في بعض المتصفحات/الأغلفة غير القياسية حيث تكون واجهة الحافظة موجودة لكن دالة النسخ الفعلية غير مفعّلة.', en: "Medium fix: copying the monthly report to the clipboard could fail in complete silence (no message, no fallback download) on some non-standard browsers/wrappers where the clipboard interface exists but the actual copy function is disabled." },
    ],
  },
  {
    version: 'v48.38',
  ...
  },
  {
    ...
    items: [
      { ar: 'إصلاح عالٍ: مسار استرجاع أرصدة المحافظ عند بدء التطبيق (من التخزين المحلي أو نسخة IndexedDB الاحتياطية) لم يكن يطبّق سقف "الحد الأقصى للمبلغ" (MAX_AMOUNT) الذي تطبّقه كل المسارات المشابهة الأخرى (استيراد نسخة احتياطية، مزامنة Drive). قيمة تالفة أو مُتلاعَب بها كانت تمر دون رفض، وقد تتسبب لاحقًا بفيض رقمي (Infinity) في مجموع ما، يُحوَّل صامتًا إلى قيمة فارغة عند الحفظ التالي فتختفي المحفظة كليًا من العرض بلا أي تحذير.', en: "High fix: the wallet-balance restore path at app boot (from local storage or the IndexedDB backup copy) wasn't applying the same \"maximum amount\" ceiling (MAX_AMOUNT) every comparable path already enforces (backup import, Drive sync). A corrupted or tampered value could pass through unrejected, and later overflow to Infinity in some sum, which silently serializes to null on the next save — making the wallet vanish from the display with no warning at all." },
      { ar: 'إصلاح عالٍ: أحد مسارين لاسترجاع سقوف الميزانية عند بدء التطبيق (من التخزين المحلي) كان يتجاوز نفس سقف MAX_AMOUNT، رغم أن المسار الآخر بنفس الدالة (من نسخة IndexedDB الأحدث) يطبّقه بشكل صحيح. الآن يستخدم كلا المسارين نفس دالة التحقق (sanitizeBudgets) فيتفقان دومًا.', en: "High fix: one of two budget-cap restore paths at app boot (from local storage) bypassed the same MAX_AMOUNT ceiling, even though the other path in the very same function (from a newer IndexedDB copy) already applied it correctly. Both paths now go through the same validator (sanitizeBudgets), so they can never disagree again." },
      { ar: 'إصلاح عالٍ: دالتا "التراجع عن التعديل" و"التراجع عن الحذف" (تُستدعيان من زر "تراجع" بإشعار التأكيد، الذي يبقى ظاهرًا لعدة ثوانٍ) كانتا الوحيدتين بين كل دوال الكتابة المالية المحمية تفتقدان قفل التداخل العابر للعمليات (_opBusy) — رغم أنهما تكتبان أرصدة ومعاملات عبر عدة نقاط انتظار تمامًا كبقية الدوال بنفس الملف. ضغط زر "تراجع" بينما عملية أخرى (كتوزيع دخل تلقائي أو مزامنة Drive) لا تزال قيد التنفيذ كان يسمح نظريًا بتداخل الكتابتين. أُضيف القفل الآن ليطابق كل الدوال المماثلة.', en: 'High fix: the "undo edit" and "undo delete" functions (invoked from the confirmation toast\'s "Undo" button, which stays on screen for several seconds) were the only two money-writing mutators across the entire app missing the cross-operation write guard (_opBusy) — even though they write balances/transactions across several awaits exactly like every other function in the same file. Tapping Undo while another operation (auto-distribution, a Drive sync) was still mid-flight could theoretically let the two writes interleave. The guard has been added to match every comparable function.' },
      { ar: 'تنظيم (بدون تغيير سلوك ظاهري): استُخرج فحص "هل هذا الاشتراك صالح الشكل؟" (المُكرَّر حرفيًا بـ٥ مواقع منفصلة عبر ٣ ملفات) إلى دالة مساعدة واحدة (isValidSubShape). كذلك أُزيلت كتلتا try/catch لا فائدة منهما فعليًا حول استدعاء مزامنة Drive عند إغلاق الصفحة/التحديث القسري — الدالة تتعامل مع كل أخطائها داخليًا ولا "تلقي" استثناءً إطلاقًا، فكانت كتلة catch ميتة لا تُنفَّذ أبدًا.', en: 'Organization (no user-visible behavior change): extracted the "is this a well-formed subscription entry?" check (duplicated verbatim at 5 separate sites across 3 files) into one shared helper (isValidSubShape). Also removed two try/catch blocks around the Drive-sync call on page-hide/force-update that could never actually catch anything — the function already handles every failure internally and never throws, so those catch blocks were dead code.' },
    ],
  },
  {
    version: 'v48.20',
  ...
  },
  {
    ...
    items: [
      { ar: 'تنظيم (بدون تغيير سلوك ظاهري): استُخرجت 5 قيم زمنية/عددية كانت مكررة كأرقام صريحة بعدة ملفات إلى ثوابت مشتركة باسم واضح (سقف طول الوصف 120 حرفًا، نافذة التراجع 5 ثوانٍ، مدة التنبيه الحرج 7 ثوانٍ، فترة انتظار نسخ IndexedDB الاحتياطية 400 مللي ثانية، فترة إعادة محاولة شعارات الإشعارات 400 مللي ثانية) — يمنع انحراف قيمتين كان يُفترض أن تتطابقا دومًا لو عُدِّلت إحداهما ونُسيت الأخرى.', en: 'Organization (no user-visible behavior change): 5 timing/threshold values that were repeated as bare numeric literals across multiple files were extracted into clearly-named shared constants (description length cap 120 chars, undo window 5s, critical-toast duration 7s, IndexedDB backup debounce 400ms, banner retry-poll interval 400ms) — preventing two values that must always agree from silently drifting apart if one is tuned and the other forgotten.' },
      { ar: 'تنظيم (بدون تغيير سلوك ظاهري): استُخرجت دالتان مساعدتان تلخصان حسابًا كان مكررًا حرفيًا بعدة مواقع — effectiveWalletBalance() لحساب رصيد المحفظة الفعلي أثناء وضع الأزمة (كان مكررًا 4 مرات)، و_daysInMonth() لحساب عدد أيام الشهر (كان مكررًا 5 مرات). كذلك استُبدلت 3 نسخ مكررة حرفيًا من منطق التوزيع الافتراضي بدالة موجودة أصلًا (_defaultDistributionForCurrentWallets) لم تكن مُستخدَمة في تلك المواضع.', en: 'Organization (no user-visible behavior change): extracted two helpers summarizing computations that were literally duplicated at several sites — effectiveWalletBalance() for a wallet\'s effective balance during crisis mode (was repeated 4x), and _daysInMonth() for days-in-a-month math (was repeated 5x). Also replaced 3 byte-identical copies of the default-distribution fallback logic with an existing helper (_defaultDistributionForCurrentWallets) that wasn\'t being reused at those sites.' },
      { ar: 'تنظيم (بدون تغيير سلوك ظاهري): أُزيل مفتاحا ترجمة غير مُستخدَمين إطلاقًا في أي مكان بالتطبيق (بقايا من نسخة سابقة لميزة ربط محفظة التتبع). أُضيفت عناوين أقسام لملف app.drive.js (كان الملف الوحيد بلا أي تقسيم داخلي رغم تعدد أقسامه الوظيفية)، وتعليق تعريفي أعلى 3 ملفات رئيسية (app.core.js، app.ui.js، app.logic.js) يوضح نطاق كل ملف ومكانه بترتيب التحميل. نُقلت دالتان كانتا في مكان غريب (scrollToTxList داخل قسم الثيم، toggleCrisis في آخر الملف بعيدًا عن الدوال المرتبطة بها) إلى مكانهما المنطقي، وأُزيلت بادئات رقم إصدار قديمة جدًا (v9.x) من عناوين بعض الأقسام لم تعد ذات معنى.', en: "Organization (no user-visible behavior change): removed two translation keys that were entirely unused anywhere in the app (leftovers from an earlier version of the tracking-wallet-link feature). Added section headers to app.drive.js (the one file with no internal sectioning despite covering several distinct concerns), and a top-of-file orientation comment to 3 central files (app.core.js, app.ui.js, app.logic.js) explaining each file's scope and place in the load order. Relocated two functions that had ended up in odd spots (scrollToTxList sitting inside the theme section, toggleCrisis stranded at the end of the file far from its related functions) to where they logically belong, and stripped very old version-number prefixes (v9.x) from a few section headers that no longer meant anything." },
    ],
  },
  {
    version: 'v48.18',
    date: '2026-07-07',
    title: { ar: 'جولة تبسيط معماري: توحيد أعلام قفل التعديل، حلقات الانتظار، وتكرار زر التراجع', en: 'Architecture simplification round: unifying edit-lock flags, wait-loops, and a duplicate undo offer' },
    items: [
      { ar: 'إصلاح عالٍ: حذف تعريف محفظة (deleteWalletDef) كان الوحيد بين كل عمليات الكتابة المالية المحمية يفتقد قفل التداخل العابر للعمليات (_opBusy) رغم أنه يكتب أرصدة وتعريفات محافظ عبر عدة نقاط انتظار (await) تمامًا كشقيقه المجاور — ما كان يسمح نظريًا بتداخل حذف محفظة مع إضافة معاملة أو مزامنة Drive في نفس اللحظة. أُضيف القفل الآن ليطابق كل نظرائه.', en: "High fix: deleteWalletDef was the one guarded money-writing mutator across the entire app missing the cross-operation write guard (_opBusy) — even though it writes balances/wallet-defs across several awaits exactly like its sibling right above it. This theoretically allowed a wallet deletion to interleave with an in-flight add-transaction or Drive sync at the same moment. The guard has been added to match every other mutator." },
      { ar: 'إصلاح متوسط: نافذة \"تراجع\" الخاصة بالحذف كانت أحيانًا تُظهر عرض تراجع إضافي وهميًا (بعدد قديم غير صحيح) بعد استخدام التراجع أو انتهاء المهلة، وذلك عند حذف معاملتين متتاليتين بسرعة — رغم أن التراجع الفعلي واحد فقط يغطي كليهما. الآن لا يُخزَّن عرض تراجع مكرر عندما يكون إجراء الحذف الجديد هو نفسه إجراء الحذف المعلَّق (لا يزال أي إجراء آخر مختلف، كتعديل ثم حذف، يُحفَظ ويُعرَض كما كان).', en: 'Medium fix: the delete-undo toast could occasionally surface an extra, stale phantom "Undo" offer (with an outdated count) after tapping undo or letting it time out, when two deletions happened in rapid succession — even though a single combined undo already covered both. A duplicate offer is no longer stashed when the new delete action is the same underlying action as the one already pending (a genuinely different action, like an edit followed by a delete, still chains back exactly as before).' },
      { ar: 'تبسيط معماري (بدون تغيير سلوك ظاهري): توحيد ثلاثة أعلام منطقية مستقلة كانت تتحكم بقفل حقول نموذج تعديل المعاملة (تحويل، مصدر توزيع، تعديل رصيد محفظة متتبَّعة) في قيمة واحدة تصف حالة القفل الفعلية بدقة أكبر — بما فيها حالة \"ضلع توزيع\" التي كانت تُفعِّل علمين بآن واحد ولم تكن موثقة بوضوح كحالة مستقلة. كذلك تم توحيد نمط حلقة الانتظار المتكرر ثلاث مرات في app.drive.js (لتأجيل استبدال الحالة أثناء نافذة/عملية مفتوحة) بدالة مساعدة واحدة، مع الحفاظ التام على الفروقات البسيطة بين كل موقع استخدام (كالمهلة الأطول عند انتظار نافذة تعارض Drive تحديدًا).', en: "Architecture simplification (no user-visible behavior change): consolidated three independent boolean flags controlling the transaction-edit form's field locks (transfer leg, distribution source, tracked-wallet balance adjustment) into a single value that more precisely captures the real lock state — including the \"distribution leg\" case, where two of the original flags were true simultaneously without that being clearly documented as its own distinct state. Also consolidated a wait-loop pattern repeated three times across app.drive.js (deferring a state swap while a modal/operation is in progress) into one shared helper, while exactly preserving each call site's small differences (like the much longer cap specifically while waiting on the Drive conflict-resolution modal)." },
    ],
  },
  {
    version: 'v48.17',
